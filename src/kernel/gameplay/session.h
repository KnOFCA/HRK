...
class GameplaySession : public IJudgmentSink {
  IGameModule& game_; TraceAudioBackend audio_;
  ...
  InputTrace* trace_=nullptr;TraceInput pendingTrace_{};uint64_t mappingSample_=0;TimeNs epochHost_=0;
  struct Observation;
  void clearInputs();
...
public:
  ...
  Error pushTraced(const InputEnvelope& event);
  void setTrace(InputTrace* trace){trace_=trace;audio_.trace(trace);}
  InputTrace* trace() const{return trace_;}
  void finishTrace(InputTrace::Scope&,Error result=Error::Ok) const;
  Error update(); void render(IRenderBackend& output,TimeNs host);
  void emit(const JudgmentResult& judgment) override;
  SessionState state() const { return state_; } TimeNs time() const;
