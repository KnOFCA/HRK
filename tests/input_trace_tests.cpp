...
int main(int argc,char** argv){
  ...
  test("TRACE-PHASES-AND-BRANCHES",[]{Fixture f;f.load();for(uint32_t p=0;p<4;++p)f.submit(static_cast<InputPhase>(p));f.poll();f.audio.advance(second);check(f.session.update()==Error::Ok);
    for(uint32_t p=0;p<4;++p)f.submit(static_cast<InputPhase>(p),second-1);f.poll();check(f.session.update()==Error::LateInput);
    for(uint32_t p=0;p<4;++p)f.submit(static_cast<InputPhase>(p),2*second);f.poll();check(f.session.update()==Error::Ok);f.session.pause();f.end();
